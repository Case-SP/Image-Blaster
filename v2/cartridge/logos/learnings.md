...
- **Wordmark text = full input title → FIXED, and it exposed a deeper bug.** Symptom: render wordmark printed the raw title ("WARP INDEPENDENT… 003"), one tile just "002". Added a `{brand_name}` sink to the render skeleton + a `slot_overrides.brand_name` injection in the orchestrator (mirrors the style_mix pattern, sourced from `objectContext[title.id].brand_name`).
- **🔴 ROOT-CAUSE BUG (pre-existing) — `readTrace is not defined` on EVERY promote.** While wiring brand_name, found that `orchestrator.js` referenced `readTrace` at the promote-context block (~:185) but only imported it in a *different* function scope (:29). So every promote run threw `readTrace is not defined`, the catch reset `objectContext = {}`, and **all parent-context carry-through was silently lost** — brand_name (→ wordmark leak), registers (→ render register was a RANDOM seeded pick, not the brand's primary!), palette_policy, render_treatment. Affects **product too** (in-situ lost its spatial facts on promote). Fix: add `readTrace` to the module-level import (`:14`). One line.
- **After the fix (verified live, Iron Bank promote):** `objectContext` populated; `brand_name="Iron Bank"` → wordmark reads "Iron Bank" cleanly; `registers` carried (primary = `swiss-modern`, apt) → render uses the brand's register not a random one; `render_treatment=flat` carried → ceiling correct per brand. Iron Bank renders show ornate/swiss marks on cream with clean "IRON BANK" wordmarks. (Older Warp renders predate the fix and still show the title.)
- **Task 4 VERDICT — variability bar met (v1).** Verified across 4 contrasting brands post-fix:
  - **Sketch:** genuinely distinct worlds (Warp bold-letterform/blue + organic + orange rosettes; Iron Bank austere swiss; New Museum contemporary geometric; Versace ornate art-deco). palette_policy + register spread both firing.
  - **Render:** carries the mark in a 4:5 split-screen system with clean brand-name wordmarks (Warp / Iron Bank / New Museum / Versace — all clean post-fix), and **register-appropriate treatment**: Versace = `material-expressive` (ornate art-deco marks, metallic grounds), Iron Bank/New Museum = `flat`. Per-brand treatment ceiling confirmed (Versace prompt carries molten-metal/chrome positives; Iron Bank stays flat).
- **Ref-budget gap closed:** `profile.ref_budget` 12 → 16 so all 16 curated sketch refs reach the model. Side effect: render promote now sends parent + up to 15 dual-source style anchors (style-consistent, acceptable; revisit if subject dilution appears).

### Task 5 — DoD + hand-back
- **Done:** style_order `[sketch, system-split-4x5]`; render composition assembles with dual-source refs; UI funnel 2 cols from FLOWS (product unchanged); sketch→render promote works (prefix, parent-as-subject, dual-source refs, 4:5, treatment ceiling, clean wordmark); ≥3 brands show distinct sketch worlds + register-appropriate render treatment.
- **Open / future tuning (not blocking):** dial dual-source 4+4 balance + render skeleton prose by eye; some Versace renders are busy (art-deco ornament fights the split layout) — may want a per-treatment skeleton hint; New Museum occasionally repeats the mark in both halves.
- **Deferred Mockup contract (stage 3, its own spec):** add a `hero-single-surface` composition named EXACTLY that (the `system-split-4x5_to_hero-single-surface` promote prefix + a `stage_resolution["hero-single-surface"]` ceiling already exist); append `hero-single-surface` to `style_order`; add `mockup` (or that name) to the logos FLOW in `app.js` (`next: { 'system-split-4x5': 'hero-single-surface' }`). The mockup uses the parent RENDER tile as subject + `references/mockup/` refs; treatment `realistic`.

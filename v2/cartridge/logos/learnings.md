...
- **Remaining Task 4 tuning (not yet done):** re-render Warp/New Museum post-fix to confirm primary-register carry across brands; judge render treatment variability for a material-expressive brand (e.g. Versace) now that render_treatment carries; tune dual-source ref balance (4+4) + render skeleton prose by eye; close the sketch ref-budget gap (16 refs, budget caps to ~12).
